...
const {
  ...
  pararRotina,
  statusRotina,
} = require('./notifications/participantNotifier');
...
app.post('/api/disc', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.rows[0].id });
    // A notificação por e-mail NÃO é disparada aqui — é a rotina
    // liga/desliga em admin.html (ver POST /api/rotina-notificacao/*)
    // que processa isso, agrupando as 3 ferramentas por e-mail.
  } catch (err) {
...
// ---------- rotina de notificação por e-mail (ao próprio participante) ----------
// Liga/desliga via botão em admin.html — nunca automática a cada envio,
// nem job sempre ativo, nem clique único (já passou pelos 3 formatos, os
// 2 primeiros descartados por pedido explícito do usuário). Enquanto
// ativa, processa periodicamente quem tem PDF pendente nas 3 tabelas,
// agrupado por e-mail (ver src/notifications/participantNotifier.js).
// Estado fica só em memória — reinicia desligada a cada boot.

app.post('/api/rotina-notificacao/ativar', requireAdmin, (req, res) => {
  const jaEstavaAtiva = !iniciarRotina();
  res.json({ ativa: true, jaEstavaAtiva });
});

app.post('/api/rotina-notificacao/desativar', requireAdmin, (req, res) => {
  const jaEstavaInativa = !pararRotina();
  res.json({ ativa: false, jaEstavaInativa });
});

app.get('/api/rotina-notificacao/status', requireAdmin, (req, res) => {
  res.json(statusRotina());
});
